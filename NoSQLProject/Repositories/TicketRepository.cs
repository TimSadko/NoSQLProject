...
namespace NoSQLProject.Repositories
{
    public class TicketRepository : ITicketRepository
    {
        ...
        public async Task CheckUpdateAsync(Ticket t)
        {
            var old = await GetByIdAsync(t.Id); // Get 'db' version of the ticket, then compare to edited version
            var filter = Builders<Ticket>.Filter.Eq("_id", ObjectId.Parse(t.Id)); // Find ticket by ID
            List<Task<UpdateResult>> tsk = new List<Task<UpdateResult>>();
            ...
            await Task.WhenAll(tsk); // wait for all updates
        }
